...
#define USER_EMAIL "ismailelghawas123@gmail.com"  // Your registered email
#define USER_PASSWORD "123456"   // Your password

/* 4. Define the Firebase storage bucket ID e.g bucket-name.appspot.com */
#define STORAGE_BUCKET_ID "fota-cbb0e.appspot.com"  // Your Firebase Storage bucket ID

/* 5. MQTT server or broker to connect on it */
...
bool taskCompleted = false;  // Flag to track if the task is completed

bool messageReceived = false;  // Flag to indicate if a message was received
char buf[2] = {0};  // Buffer to hold two hex digits
char packet[71];  // Buffer for the packet
static uint32_t currentAddress = 0x08003000;  // Starting address
...
void setup()
{
    ...
    Serial2.begin(115200, SERIAL_8N1, 16, 17);  // Initialize UART2 on GPIO 16 (RX) and GPIO 17 (TX)

    WiFi.begin(WIFI_SSID, WIFI_PASSWORD);  // Start connecting to Wi-Fi

    Serial.print("Connecting to Wi-Fi");  // Print message to Serial Monitor
    unsigned long ms = millis();  // Get current time in milliseconds
    ...
    fbdo.setBSSLBufferSize(4096, 1024);  // Set buffer size for BSSL
    config.fcs.download_buffer_size = 2048;  // Set download buffer size for Firebase Storage
    Firebase.begin(&config, &auth);  // Initialize Firebase

    client.setServer(mqtt_server, 1883);  // Set MQTT server and port
    client.setCallback(MQTT_Callback);  // Set callback function for MQTT messages

    pinMode(2, OUTPUT);  // Set GPIO pin 2 as output
}

void loop()
{
    if (!client.connected()) { reconnect(); }  // Reconnect to MQTT broker if not connected
...
{ 
    while (!client.connected()) 
    {
        ...
        if (client.connect("ESPClient")) 
        {
            ...
            client.subscribe("Fota/test");  // Subscribe to the topic
        } 
        else 
        {
...
void MQTT_Callback(char* topic, byte* payload, unsigned int length) {
  byte message = 0;  // Variable to store the received message
  Serial.print("Message arrived [");
  Serial.print(topic);  // Print the topic of the received message
  Serial.print("]: ");
  message = payload[0];  // Get the first byte of the payload
  Serial.println(message);  // Print the received message

  switch(message) {
    //case 'E':  Send_CMD_Erase_Flash();            break;  // Placeholder for erase command
    case 'U':  Send_CMD_Upload_Application();     break;  // Upload application if 'U' is received
  }
}
...
void Send_CMD_Upload_Application() {
  if (Firebase.ready()) {  // Check if Firebase is ready
    ...
    if (!Firebase.Storage.download(&fbdo, STORAGE_BUCKET_ID /* Firebase Storage bucket id */, "Application1.txt" /* path of remote file stored in the bucket */, "/new.txt" /* path to local file */, mem_storage_type_flash /* memory storage type, mem_storage_type_flash and mem_storage_type_sd */, fcsDownloadCallback /* callback function */)) {
      Serial.println(fbdo.errorReason());  // Print error if download fails
    }
    // Reading the file after it has been downloaded
    //readFile("/new.bin");
    BL_SendData();  // Send data over UART2
  }
}

void BL_SendData() {
  File file = LittleFS.open("/new.txt", "r");  // Open the downloaded file for reading
  if (!file) {
    Serial.println("Failed to open file");  // Print error if file opening fails
    return;
  }
  ...
  while (file.available()) {
    ...
    // Set fixed parts of the packet
    packet[0] = 70;
    packet[1] = 0x14;
    *(uint32_t *)(&(packet[2])) = currentAddress;  // Set the starting address
    packet[6] = 64;

    // Read 64 bytes from the file and convert each pair of hex digits to a byte
    for (counterByte = 0; counterByte < 64 && file.available(); ++counterByte) {
      file.readBytes(buf, 2);
      uint8_t hexData = strtol(buf, NULL, 16);
      packet[7 + counterByte] = hexData;  // Store the converted data in the packet
    }

    // Send the packet over UART2
    for (int i = 0; i < 7; ++i) {
    ...
    for (int i = 0; i < 64; ++i) {
      Serial2.write(packet[7 + i]);
      Serial.printf("\npacket[%i] = %x", 7 + i, packet[7 + i]);
    }

    // Optionally wait for acknowledgment from STM32
    ...
    memset(packet + 7, 0, 64);

    // Update the address for the next packet
    currentAddress += 64;
  }
